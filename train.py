...
def get_params():
    ...
    parser.add_argument(
        "--ss", type=str, default="ddpm"
    )  # help='sample strategy', ddpm, multi_diffusion, one_diffusion
...
def default_config(data="AIR_BJ"):
    ...
    gpu_id = GPU().get_usefuel_gpu(
        max_memory=6000, condidate_gpu_id=[0, 1, 2, 3, 4, 6, 7, 8]
    )
...
def evals(model, data_loader, epoch, metric, config, clean_data, mode="Test"):
    ...
    for i, batch in enumerate(data_loader):
        ...
        future, history, pos_w, pos_d = to_device(
            batch, config.device
        )  # target:(B,T,V,1), history:(B,T,V,1), pos_w: (B,1), pos_d:(B,T,1)

        x = torch.cat((history, future), dim=1).to(
            config.device
        ...
        x, x_hat = clean_data.reverse_normalization(
            x
        ), clean_data.reverse_normalization(x_hat)
        x_hat = x_hat.detach()
        f_x, f_x_hat = (
            x[:, :, :, -config.model.T_p :],
            x_hat[:, :, :, :, -config.model.T_p :],
        )  # future

        _y_true_ = f_x.transpose(1, 3).cpu().numpy()  # y_true: (B, T_p, V, D)
        _y_pred_ = (
            f_x_hat.transpose(2, 4).cpu().numpy()
    ...
    if config.nni:
        nni.report_intermediate_result(metric.metrics["mae"])
...
def main(params: dict):
    ...
    train_dataset = TrafficDataset(
        clean_data,
        (0 + config.model.T_p, config.data.val_start_idx - config.model.T_p + 1),
        config,
    )
    train_loader = torch.utils.data.DataLoader(
        train_dataset, config.batch_size, shuffle=True, pin_memory=True
    )

    val_dataset = TrafficDataset(
        clean_data,
        (
            config.data.val_start_idx + config.model.T_p,
    ...
    test_dataset = TrafficDataset(
        clean_data, (config.data.test_start_idx + config.model.T_p, -1), config
    )
    ...
    config.forecast_path = forecast_path = (
        config.PATH_FORECAST + config.trial_name + ".pkl"
    )
    ...
    config.logger.write(
        f"Num_of_parameters:{sum([p.numel() for p in model.parameters()])}\n",
        is_terminal=True,
    )
    ...
    new_log_path = os.path.join(
        log_file, f"[{config.data.name}]mae{min(metric_lst):7.2f}+{log_name}"
    )
    import shutil
